...
for sample in $samples; do
    for seed in $seeds; do
        ...
        for model in $models; do
            python sosci_simlearn/train.py --train-path=$train_path --val-path=$val_path --output-dir=$output_dir --max-epochs=$epochs --model-name=$model --sample-n=$sample --seed=$seed
        done
    done
done